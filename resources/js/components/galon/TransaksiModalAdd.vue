<template>
  <basic-modal v-if="showModal" @close="showModal = false">
    <h5 slot="header">Tambah Transaksi</h5>
    <div slot="body">
      <form
        id="addForm"
        @submit.prevent="addTransaksi"
...
export default {
  ...
  methods: {
    async addTransaksi() {
      const { data } = await this.form.post("api/transaksi");
      this.$store.commit("transaksi/addTransaksi", data);
      this.form.reset();
