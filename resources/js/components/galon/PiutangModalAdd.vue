<template>
  <basic-modal v-if="showModal" @close="showModal = false">
    <h5 slot="header">Tambah Piutang</h5>
    <div slot="body">
      <form
        id="addForm"
        @submit.prevent="addPiutang"
...
export default {
  ...
  methods: {
    async addPiutang() {
      const { data } = await this.form.post("api/piutang");
      this.$store.commit("piutang/addPiutang", data);
      this.form.reset();
