<template>
  <basic-modal v-if="showModal" @close="showModal = false">
    <h5 slot="header">Tambah DetailTransaksi</h5>
    <div slot="body">
      <form
        id="addForm"
        @submit.prevent="addDetailTransaksi"
...
export default {
  ...
  methods: {
    async addDetailTransaksi() {
      const { data } = await this.form.post("api/detailTransaksi");
      this.$store.commit("detailTransaksi/addDetailTransaksi", data);
      this.form.reset();
